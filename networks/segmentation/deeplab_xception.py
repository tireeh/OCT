...
class Xception(nn.Module):
    ...
    """
    def __init__(self, inplanes=3, os=16, pretrained=False, ignore_conv1 = True):
        ...
        if pretrained:
            self.__load_xception_pretrained(ignore_conv1)
    ...
    def __load_xception_pretrained(self, ignore_conv1 = True):
        ...
        for k, v in pretrain_dict.items():
            if k in state_dict:
                if ignore_conv1 and k.startswith('conv1'):
                    continue
...
class DeepLabv3_plus_xception(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=21, os=16, pretrained=False, _print=True, ignore_conv1 = True):
        if _print:
        ...
        # Atrous Conv
        self.xception_features = Xception(nInputChannels, os, pretrained, ignore_conv1)
...
if __name__ == "__main__":
    model = DeepLabv3_plus_xception(nInputChannels=3, n_classes=21, os=8, pretrained=True, _print=True)
    model.eval()
