...
from .switchable_norm import SwitchNorm
...
class DoubleConv(nn.Module):
  ...
  def __init__(self, in_ch, out_ch):
    ...
    self.conv = nn.Sequential(
      CoordConv2d(in_channels=in_ch + 3, out_channels=out_ch, kernel_size=3, padding=1, bias=False),
      sn.SwitchNorm(out_ch),
      nn.ELU(),
      CoordConv2d(in_channels=out_ch + 3, out_channels=out_ch, kernel_size=3, padding=1, bias=False),
      sn.SwitchNorm(out_ch),
      nn.ELU(),
    )
...
class UpConv(nn.Module):
  def __init__(self, in_ch, out_ch, bilinear=False):
    ...
    if bilinear:
      ...
      self.up = CoordConvTranspose2d(in_channels=in_ch // 2 + 3, out_channels=in_ch // 2, kernel_size=2, stride=2,
                                     bias=False)
      self.sn = sn.SwitchNorm(in_ch // 2)
    self.conv = DoubleConv(in_ch, out_ch)

  def forward(self, x1, x2):
    x1 = self.sn(self.up(x1))
...
class OutConv(nn.Module):
  def __init__(self, in_ch, out_ch):
    ...
    self.conv = CoordConv2d(in_channels=in_ch + 3, out_channels=out_ch, kernel_size=1)
...
class UNet_coordconv_sn_elu(nn.Module):
  def __init__(self, n_channels, n_classes):
    super(UNet_coordconv_sn_elu, self).__init__()
